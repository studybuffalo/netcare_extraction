...
function normalizeMedicationData(data) {
    // Takes the medication data and normalizes it
    console.log(data);
...
function extractMedicationData(rows) {
    ...
    return normalizeMedicationData(medicationData);
}
...
function copyResult(msg) {
    ...
    veil.appendChild(responseInput);
    responseInput.select();
    document.execCommand("Copy");

    // Remove the element
    veil.removeChild(responseInput);
}
...
function processMedicationExtraction() {
    ...
    if (validation) {
        ...
        // Copy the medicationData to the clipboard
        copyResult(medicationData);
...
function generateDOMVeil() {
    // Veil covers page to capture any user clicks & hold other content
...
function generateDOMMedicationExtraction() {
    // Generate the veil
    let veil = generateDOMVeil();
...
browser.runtime.onMessage.addListener((request, sender, sendResponse) => {
    ...
    if (extractionType === "LABS") {
        ...
        // Inject the require HTML content
        generateDOMMedicationExtraction();
    }
});
